...
def create_sinusoidal_movement(duration_sec, sample_period, x_velocity, y_start_offset, start_time):
...
sample_period = 0.5  # [s]
velocity_x = 1  # [m/s]
time_W2A, pq_arr_W2A = create_sinusoidal_movement(duration, sample_period,
                                              velocity_x, y_start_offset=0.0, start_time=0.1)
time_W2B, pq_arr_W2B = create_sinusoidal_movement(duration, sample_period,
                                              velocity_x, y_start_offset=2.0, start_time=0.35)
...
def interpolate_pq(query_time, t_arr, pq_array):
    ...
    pq : array-like, shape (7,)
        Interpolated position and orientation quaternion: (x, y, z, qw, qx, qy, qz)
    """
...
origin_of_B_in_A = pt.transform(query_transform_A2B, pt.vector_to_point([0, 0, 0]))
